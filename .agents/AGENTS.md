...
5. **Read-Only /ask Command (Context-Aware Q&A):** If the user sends a message starting with `/ask`, absolutely do not perform actions like modifying code or executing terminal commands. Provide only answers and explanations based on the local workspace context (`03_Coding_Rulebook`, `01_Academic_Papers`, `BSMA_Master_Coding_Sheet.xlsx`).
   - **ZERO HALLUCINATION:** If the information is not present in the workspace, you MUST say: "I cannot find this information in the current workspace files." Do NOT use external pre-trained knowledge to answer data/coding questions.
   - **MANDATORY CITATION:** ALWAYS return a `[SOURCES]: <Exact file path and location>` block at the end of your answer.
...
22. **Task Orchestration & Rollback Strategy:** When asked to plan or perform a complex task (e.g., data extraction, file modification), the AI must generate a step-by-step Work Breakdown Structure (WBS). If the task involves modifying or deleting files (Write/Delete), the AI MUST include **"Step 0: Create a timestamped backup in `99_Archives_and_Backups/`"** before any other actions, and evaluate the risk (Low/High/Critical). The AI must wait for explicit user approval before proceeding.
