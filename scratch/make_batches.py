...
batch_file = 'g:/My Drive/UCL/BSMA/BSMA ANTIGRAVITY/scratch/batch_chunk_5.json'
papers = json.load(open(batch_file, encoding='utf-8'))
...
out_dir = 'g:/My Drive/UCL/BSMA/BSMA ANTIGRAVITY/scratch/batches'
...
for i in range(0, len(papers), batch_size):
    ...
    subagents = []
    for p in batch:
        prompt = base_prompt + f"Now review `{p['id']}`. The PDF is at `{p['pdf']}`. Output to `{p['out']}`."
        subagents.append({
            'TypeName': 'bsma_reviewer_v3',
            'Role': f"Validation2 for {p['id']}",
            'Prompt': prompt
        })
    with open(f'{out_dir}/batch_{i//batch_size}.json', 'w', encoding='utf-8') as f:
        json.dump(subagents, f, indent=2)

print('Generated', len(papers)//batch_size + (1 if len(papers)%batch_size else 0), 'batches.')
